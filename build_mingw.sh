...
rm -rf "$ROOT/build_mingw"
rm -f "$ROOT/releases"/LR_JS-$VERSION-mingw-*.zip
mkdir -p "$ROOT/build_mingw" "$ROOT/releases"
...
EOF
    ...
    (
        ...
        || { echo "[FAIL] $arch configure failed."; tail -30 "$bdir/configure.log"; exit 1; }

    # Build (parallel)
    nproc=$(nproc 2>/dev/null || echo 4)
    cmake --build "$bdir" --config Release -j "$nproc" > "$bdir/build.log" 2>&1 \
        || { echo "[FAIL] $arch build failed."; tail -30 "$bdir/build.log"; exit 1; }

    # Verify
    exe="$bdir/bin/lr_js.exe"
    if [ ! -f "$exe" ]; then
        exe=$(find "$bdir" -name "lr_js.exe" -type f 2>/dev/null | head -1)
    fi
    if [ -z "$exe" ] || [ ! -f "$exe" ]; then
