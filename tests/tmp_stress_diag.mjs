// IOME586 结果缓存压力测试脚本
// 运行方式：node iome586_stress_test.mjs 或 lr_js -m iome586_stress_test.mjs
// 预期冷启动 800-1200ms，热启动 50-150ms（缓存命中）
...
console.log(`异步累加值: ${asyncAccum} (预期 ~4950)`);
console.log(`Atomics 操作数: ${atomicsOps}`);
console.log(`SAB 校验和: ${sum}`);console.log('REACHED END'); 
