...
| **Windows** | x86_64, x86, aarch64 | MSVC 2022+, [MinGW-w64](https://www.mingw-w64.org/) (**recommended**, GCC computed goto 4-6× faster) | Windows 7+ |
...
| Function | Description |
|----------|-------------|
| `int lr_event_loop_run(LR_Runtime *rt)` | Run the event loop until no pending tasks, returns 0 on success, -1 on error |
| `int lr_event_loop_run_timeout(LR_Runtime *rt, int timeout_ms)` | Run the event loop for at most `timeout_ms` milliseconds, returns 0 on success, -1 on error |
| `int lr_event_loop_pending(LR_Runtime *rt)` | Check if there are pending tasks in the event loop |
| `void lr_event_loop_stop(LR_Runtime *rt)` | Stop the event loop on the next iteration |
...
The artifact is `releases/LR_JS-0.2.0-macos-{x86_64,arm64}.tar.gz`, containing `lib/liblr_js.a`, `lib/liblr_js.dylib`, `bin/lr_js`, and `lr_js.h`.
...
|---------|------|-------------|
| 0.2.0 | 2026-08 | **Bytecode VM maturity**: direct/indirect threaded bytecode VM (GCC/Clang computed goto, MSVC switch-based dispatch); IOME586 memo cache pure-function result caching (cold hit rate 56.3%, warm hit rate 71.2%); fib20 warm run 29.7× faster than V8; dot/sum/mixed/impure pure-computation loads 1.5×~3.5× faster than V8; `lr_memory_usage()` exposed as JS global function for memory stats; `lr_js_version` / `__LR_PARALLEL_THREADS__` global variables; fixed negative memory usage reporting; 16-thread parallel execution; AST tree-walk interpreter retired; thread pool, renderer bridge, multi-platform support (FreeBSD/OpenBSD/NetBSD/Android/iOS); new `LR_Config` fields: `enable_thread_pool`, `thread_pool_size`, `enable_sandbox`, `enable_perf_optimizations`, `enable_renderer`, `module_paths`, I/O redirection |
